...
export default function DoctorAnalytics() {
  ...
  const patients = new Set(appointments.map(a => a.patient_id)).size
  const doneRate = total > 0 ? Math.round((completed.length / total) * 100) : 0
  ...
  appointments.forEach(a => {
    ...
    if (a.status === "completed") {
      monthlyMap[key].revenue += Number(a.doctor_share || 0);
    }
  });
