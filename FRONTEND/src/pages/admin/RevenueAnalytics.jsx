...
export default function RevenueAnalytics() {
  ...
  const totalRevenue = completed.reduce(
    (sum, a) => sum + Number(a.admin_share || 0),
    0
  );
  const doctorRevenue = completed.reduce(
    (sum, a) => sum + Number(a.doctor_share || 0),
    0
  );
  ...
  completed.forEach(a => {
    ...
    monthMap[key].revenue += Number(a.admin_share || 0);
    monthMap[key].count++
  })
  ...
  completed.forEach(a => {
    const doc = doctors.find(
      d => Number(d.id) === Number(a.doctor_id)
    );
    const spec = doc?.specialization || 'Other'
    specMap[spec] =
      (specMap[spec] || 0) +
      Number(a.admin_share || 0);
  })
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Avg Consultation', value: `₹${avgFee}`, icon: TrendingUp, color: 'bg-teal-600' },
          {
            ...
            value: `₹${doctorRevenue.toLocaleString()}`,
            icon: DollarSign,
            color: 'bg-blue-600'
          }
