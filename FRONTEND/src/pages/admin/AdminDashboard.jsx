...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true)
        const [appts, allUsers, docs] = await Promise.all([
          getAppointments(),
          getUsers(),
          getDoctors(),
        ])
  ...
  const revenue = appointments
    ...
    .reduce((sum, a) => sum + Number(a.admin_share || 0), 0);
            ...
            <div className="space-y-3">
              {recentAppts.map(appt => {
                const doc = doctors.find(
                  d => Number(d.id) === Number(appt.doctor_id)
                )
                const patient = users.find(
                  u => Number(u.id) === Number(appt.patient_id)
                )
                return (
